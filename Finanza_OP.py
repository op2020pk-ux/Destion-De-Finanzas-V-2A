...
class FinanzasPro:
    def __init__(self, root, usuario_id, nombre_usuario):
        ...
        # Verificar/Crear columnas necesarias si no existen
        try:
...
# ******************** MODULO (PESTAÑA MANUAL E INFORMACION TECNICA) ******************** #
    ...
    def setup_tab_manual(self):
        ...
        txt_manual = tk.Text(text_frame, font=("Segoe UI", 11), wrap="word", padx=20, pady=20, yscrollcommand=scrollbar.set, relief="flat", bg="#f4f7f6", fg="#2c3e50")
        txt_manual.pack(side="left", fill="both", expand=True)
        scrollbar.config(command=txt_manual.yview)

        manual_contenido = """
GUÍA DE USO - SISTEMA DE FINANZAS PRO v2.5
----------------------------------------------------------------------------------------------------
Este sistema está diseñado para facilitar el seguimiento detallado de sus movimientos financieros. 
...
2. GRÁFICOS: 
   ...
   • A la IZQUIERDA: Listado de todos sus ingresos por categoría con sus totales acumulados.
   • A la DERECHA: Listado de todos sus egresos por categoría con sus totales acumulados.

3. EXPORTAR: 
   • Genera un PDF detallado de sus movimientos. Puede elegir dónde guardarlo en Configuración.

4. CONFIGURACIÓN:
   • Cambie su contraseña.
   • Personalice la imagen de inicio.
   • Defina la carpeta predeterminada para sus reportes.
----------------------------------------------------------------------------------------------------
© 2025 - Soporte: op2020pk@gmail.com
        """
...
# ******************** MODULO (PESTAÑA INFORMACION TECNICA) ******************** #
    ...
    def setup_tab_informacion(self):
        ...
        tk.Label(container, text="INFORMACIÓN TÉCNICA DEL SISTEMA", font=("Segoe UI", 16, "bold"), bg="white", fg=COLOR_PRIMARY).pack(pady=(0, 20))
        detalles_frame = tk.LabelFrame(container, text=" Detalles del Software ", bg="white", font=("Segoe UI", 10, "bold"), padx=20, pady=20)
        detalles_frame.pack(fill="x")
        info_tecnica = [("Nombre del Software:", "Finanzas Pro Daily"), ("Versión:", "2.5.0 (Build 2025)"), ("Lenguaje:", "Python 3.x"), ("Interfaz Gráfica:", "Tkinter"), ("Base de Datos:", "SQLite3"), ("Librerías Extra:", "Pillow, Matplotlib, FPDF")]
        for i, (label, value) in enumerate(info_tecnica):
            tk.Label(detalles_frame, text=label, font=("Segoe UI", 10, "bold"), bg="white").grid(row=i, column=0, sticky="w", pady=2)
            tk.Label(detalles_frame, text=value, font=("Segoe UI", 10), bg="white", fg=COLOR_ACCENT).grid(row=i, column=1, sticky="w", padx=10, pady=2)
        estado_frame = tk.LabelFrame(container, text=" Estado de la Conexión ", bg="white", font=("Segoe UI", 10, "bold"), padx=20, pady=20)
...
# ******************** MODULO (PESTAÑA INFORMACION DEL CREADOR) ******************** #
    ...
    def setup_tab_creador(self):
        for widget in self.tab_creador.winfo_children(): widget.destroy()
        
        container = tk.Frame(self.tab_creador, bg="white", padx=50, pady=40)
        container.pack(fill="both", expand=True)
        ...
        tk.Label(bio_frame, text="Informacion Sobre el Creador", font=("Segoe UI", 13, "bold"), bg="#f4f7f6", fg=COLOR_PRIMARY).pack(anchor="w", pady=(0, 10))
        
        bio_texto = ("Desarrollador experto en soluciones tecnológicas y financieras. Finanzas Pro v2.5 "
             "es una muestra de mi compromiso con la calidad y la eficiencia. "
             "Especializado en crear software a medida que optimiza procesos y simplifica la toma de decisiones.\n\n"
             "¿Buscas una solución personalizada para tu empresa o proyecto? "
             "Estoy disponible para desarrollar sistemas de inventario, ventas y automatizaciones a medida. "
             "¡Contáctame y hagamos realidad tu idea!")
        
        tk.Label(bio_frame, text=bio_texto, font=("Segoe UI", 10), bg="#f4f7f6", 
                 justify="left", wraplength=800).pack(anchor="w")

        # Panel de contacto y redes
        ...
        tk.Label(left_contact, text="🔗 Canales De Contacto 🔗", font=("Segoe UI", 12, "bold"), bg="white", fg=COLOR_PRIMARY).pack(anchor="w", pady=5)
        
        contactos = [
            ("Email Soporte:", "op2020pk@gmail.com"),
            ("Email Personal:", "op2020pk@gmail.com"),
            ("Whasapp:", "+58 412-367-9938"),
            ("Ubicación:", "Venezuela")
        ]
        
        for label, value in contactos:
            f = tk.Frame(left_contact, bg="white")
            f.pack(anchor="w", pady=2)
            tk.Label(f, text=label, font=("Segoe UI", 10, "bold"), bg="white").pack(side="left")
            tk.Label(f, text=value, font=("Segoe UI", 10), bg="white", fg=COLOR_ACCENT).pack(side="left", padx=5)

        right_contact = tk.Frame(contact_frame, bg="white")
        right_contact.pack(side="right", fill="both", expand=True)
        
        tk.Label(right_contact, text="🔗 Redes Profesionales 🔗", font=("Segoe UI", 12, "bold"), bg="white", fg=COLOR_PRIMARY).pack(anchor="w", pady=5)
        
        redes = [
            ("GitHub:", "https://github.com/op2020pk-ux"),
            ("Facebook:", "Programador De Sistemas De Escritorio En Python"),
            ("Twitter/X:", "")
        ]
        
        for label, value in redes:
            f = tk.Frame(right_contact, bg="white")
            f.pack(anchor="w", pady=2)
            tk.Label(f, text=label, font=("Segoe UI", 10, "bold"), bg="white").pack(side="left")
            tk.Label(f, text=value, font=("Segoe UI", 10), bg="white", fg=COLOR_SUCCESS).pack(side="left", padx=5)
